...
defineOptions({ name: 'RecentTasks' });
...
const columns = computed<DataTableColumns<Api.Task.TaskItem>>(() => {
  const base: DataTableColumns<Api.Task.TaskItem> = [
    {
      key: 'id',
...
<template>
  <NCard :title="title" :bordered="false" size="small" class="card-wrapper">
    <template #header-extra>
      <NButton text type="primary" size="small" @click="routerPushByKey('task')">
    ...
    <NDataTable
      ...
      :pagination="false"
      :row-key="row => row.id"
      :scroll-x="compact ? 760 : 900"
      size="small"
      :max-height="compact ? 220 : undefined"
    >
      <template #empty>
